...
if ! command -v subfinder -up &> /dev/null; then
    ...
    mv ./tools/subfinder /usr/bin/ && chmod a+x /usr/bin/subfinder
fi
...
if ! command -v nuclei -up &> /dev/null; then
    ...
    # https://github.com/projectdiscovery/nuclei/releases/download/v3.3.4/nuclei_3.3.4_linux_amd64.zip
    mv ./tools/nuclei /usr/bin/ && chmod a+x /usr/bin/nuclei
fi
...
if ! command -v httpx -up &> /dev/null; then
    ...
    mv ./tools/httpx /usr/bin/ && chmod a+x /usr/bin/httpx
fi
